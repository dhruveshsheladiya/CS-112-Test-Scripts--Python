...
# 
#     python3 tester1p.py yourfilehere.py
#
# (if you're on windows, instead use "python tester1p.py yourfilehere.py").
# 
...
def comp(expected, got):
	for i in range(max(len(expected),len(got))):
		if i >= len(expected) or i >= len(got) or expected[i] != got[i]:
			return "\tExpected:\n<<" + \
				expected[findStart(expected, i-1):findEnd(expected, i+1)] + \
				">>\n\n\tGot:\n<<" + \
				got[findStart(got, i-1):findEnd(got, i+1)] + \
				'>>'
	return ""
...
class Test:
	...
	def run(self, outfilename = "results.txt", lenient=False):
		...
		try:
			...
			got = numbersOnSingleLine(got)
			self.expected = numbersOnSingleLine(self.expected)
			...
			if got!=self.expected:
				...
				print(("#"*banner_width)+"\n"
					  ...
					  +'"""\n\n    got:\n'
					  +'"""'+(repr(got))+'"""\n'
					  +"\n\n\tArea of FIRST differing output:\n\n"
